...
def main(args):
    ...
    extendexactmatch_train_path = os.path.join(root, 'ExtendExactMatch/eeml1.txt')
    ...
    eem_train_data = load_extendexactmatch(extendexactmatch_train_path)
    #eeml1_dev_data = load_extendexactmatch(cola_dev_path)
    #eeml1_test_data = load_extendexactmatch(cola_test_path, is_train=False)
    logger.info('Loaded {} ExtendExactMatch train samples'.format(len(eem_train_data)))
    #logger.info('Loaded {} ExtendExactMatch dev samples'.format(len(cola_dev_data)))
    #logger.info('Loaded {} ExtendExactMatch test samples'.format(len(cola_test_data)))
    ...
    eem_train_fout = os.path.join(mt_dnn_root, 'eem_train.json')
    build_data(eem_train_data, eem_train_fout)
    logger.info('done with ExtendExactMatch')
