...
def load_extendexactmatch(file, header=False, is_train=True, positive_label=['Excellent', "Perfect"], negative_label=["Bad", "Fair", "Good"]):
    rows = []
    cnt = 0
    with open(file, encoding="utf8") as f:
        for line in f:
            ...
            score = 0.0
            if is_train:
                if (blocks[-1] in positive_label) or (blocks[-1] in negative_label):
                    if (blocks[-1] in positive_label):
                        score = 1.0
                    else:
                        score = 0.0
                    sample = {'uid': cnt, 'premise': blocks[-3],'hypothesis': blocks[-2], 'label': score}
...
def submit(path, data, label_dict=None):
    #header = 'index\tlabel\tprediction\tscore'
    with open(path ,'w') as writer:
        uids, labels, predictions, scores = data['uids'], data['labels'], data['predictions'], data['scores']
        #writer.write('{}\n'.format(header))
        assert len(predictions) == len(uids)
        # sort label
        paired = [(int(uid), labels[idx], predictions[idx], scores[idx]) for idx, uid in enumerate(uids)]
        paired = sorted(paired, key=lambda item: item[0])
        for uid, label, pred, score in paired:
            if label_dict is None:
                writer.write('{}\t{}\t{}\t{}\n'.format(uid, label, pred, '\t'.join(str(s) for s in score)))
            else:
                assert type(pred) is int
                writer.write('{}\t{}\t{}\t{}\n'.format(uid, label, label_dict[pred], '\t'.join(str(s) for s in score)))
